...
def score_finetuned(
    accuracy: float, min_acc: float = 0.30, max_acc: float = 0.50
) -> float:
    """
...
def score_prompting(
    prompting_acc: float, finetuned_acc: float, required_boost: float = 0.04
) -> float:
    """
...
def grade_submission(
    ...
    total_score = (
        finetuned_weight * finetuned_score + prompting_weight * prompting_score
    )
...
def main():
    ...
    parser.add_argument(
        "--finetuned", type=str, help="Path to fine-tuned predictions JSON"
    )
    parser.add_argument(
        "--prompting", type=str, help="Path to prompting predictions JSON"
    )
    parser.add_argument(
        "--validation", type=str, required=True, help="Path to validation data JSON"
    )
    parser.add_argument(
        "--submissions_dir", type=str, help="Directory containing submissions"
    )
    ...
    if args.submissions_dir:
        ...
        if not args.finetuned or not args.prompting:
            parser.error(
                "Must provide --finetuned and --prompting, or --submissions_dir"
            )
