...
class RotaryPositionEmbedding(nn.Module):
    ...
    def __init__(self, d_model: int, max_seq_len: int, theta: float = 10000.0):
        ...
        inv_freq = torch.tensor(
            [1 / (theta ** (2 * i / self.d_model)) for i in range(self.d_model // 2)]
        )
...
def count_flops_per_token(
    ...
    attention_flops = num_heads * (context_length**2) * d_k * 4
    ffn_flops = num_heads * context_length * (6 * d_k * d_ff + d_ff**2)
    output_flops = context_length * d_model * vocab_size
    return (attention_flops + ffn_flops) * num_layers + output_flops
...
def estimate_memory_bytes(
    ...
    return ((attention + ffn) * num_layers + output) * dtype_bytes
