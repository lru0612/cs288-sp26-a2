...
# def get_pairs_counter(word: tuple[bytes, ...]):
#     """Get all adjacent pairs in a word (tuple of byte tokens)."""
#     pair_count = Counter()
#     for i in range(len(word) - 1):
#         pair_count[(word[i], word[i + 1])]+=1
#     return pair_count
def get_pairs_tuple(word: tuple[bytes, ...]) -> set[tuple[bytes, bytes]]:
    """Get all adjacent pairs in a word (tuple of byte tokens)."""
    pairs = set()
    for i in range(len(word) - 1):
...
def train_bpe(
    ...
    vocab = {key: value.encode("utf-8") for key, value in enumerate(special_tokens)}
    for i in range(256):
