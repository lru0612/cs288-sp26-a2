...
from part4.datasets import PretrainingDataset, create_pretraining_dataloader, create_qa_dataloader
...
CONFIGS = {
    ...
    "small": {
        ...
        "pretrain_val_data": Path(__file__).parent / "fixtures/tinystories_100k_val.txt",
...
def pretrain_lm(
    ...
    val_dataloader = None
    val_path = config.get("pretrain_val_data")
    ...
    if val_dataloader:
        print(f"  Val   sequences: {len(val_dataset)}  (file: {val_path})")
...
def finetune_qa(
    ...
    train_config = TrainingConfig(
        ...
        use_amp=device == "cuda",   # 开启混合精度，节省显存
        use_wandb=use_wandb,
    )
...
Examples:
    ...
    if use_wandb:
        wandb.init(
                project="cs288-proj2",
                name=f"{config_name}-{time.strftime('%Y%m%d-%H%M')}",
                config={
                    **config,
                    "pretrain_data": str(config["pretrain_data"]),
